...
interface AdminUpdateTripDetailModalProps {
  open: boolean;
  onClose: () => void;
  tripDetail: any;
  onSuccess?: () => void;
...
export function AdminUpdateTripDetailModal({
  open,
  onClose,
  tripDetail,
  onSuccess,
}: AdminUpdateTripDetailModalProps) {
  const [adminAmount, setAdminAmount] = useState<string>(
    tripDetail?.admin_amount?.toString() || ""
  );
  const [adminNotes, setAdminNotes] = useState<string>(
    tripDetail?.admin_notes || ""
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const existingFileIds = tripDetail?.admin_file_ids || [];
      const newFileIds = uploadedFiles.map(f => f.id);
      const allFileIds = [...existingFileIds, ...newFileIds];
      
      if (allFileIds.length > 0) {
        data.admin_file_ids = allFileIds;
      }

      await travelService.updateTripDetailAdmin(tripDetail.id, data);
      toast.success("Trip detail updated successfully");
      onSuccess?.();
      onClose();
      ...
      toast.error(error.response?.data?.message || "Failed to update trip detail");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>Update Trip Detail (Admin)</DialogTitle>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {tripDetail?.admin_file_ids && tripDetail.admin_file_ids.length > 0 && (
              <div className="mt-3">
                <p className="text-sm font-medium text-gray-600">Existing Files:</p>
                <div className="flex flex-wrap gap-2 mt-1">
                    {tripDetail.admin_file_ids.map((fileId: string, idx: number) => (
                        <div key={fileId} className="bg-gray-100 px-2 py-1 rounded text-xs flex items-center">
