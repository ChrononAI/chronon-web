...
const MileagePage = ({ 
  ...
  const loadMileagePolicies = async () => {
    ...
    try {
      ...
      const mileagePolicies = allPolicies.filter((policy: Policy) => {
        ...
        return policyName.includes('mileage') ||
  ...
  const handleInputChange = (field: string, value: string | boolean) => {
    setFormData(prev => {
      ...
      if (field === 'vehiclesType') {
        calculateMileageCost(updatedData.startLocationId, updatedData.endLocationId, value as string, updatedData.isRoundTrip)
      } else if (field === 'isRoundTrip') {
        calculateMileageCost(updatedData.startLocationId, updatedData.endLocationId, updatedData.vehiclesType, value as boolean)
  ...
  const handleStartLocationSelect = (place: PlaceSuggestion) => {
    setFormData(prev => {
      ...
      calculateMileageCost(place.place_id, updatedData.endLocationId, updatedData.vehiclesType, updatedData.isRoundTrip)
      return updatedData
    })
  }
  ...
  const handleEndLocationSelect = (place: PlaceSuggestion) => {
    setFormData(prev => {
      ...
      calculateMileageCost(updatedData.startLocationId, place.place_id, updatedData.vehiclesType, updatedData.isRoundTrip)
      return updatedData
    })
  }

  const calculateMileageCost = async (originId: string, destinationId: string, vehicle: string, isRoundTrip: boolean = false) => {
    ...
    try {
      ...
      if (costData) {
        ...
        const baseDistance = costData.distance?.distance || parseFloat(costData.distance?.text?.replace(/[^\d.]/g, '') || '0')
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.startLocation || !formData.endLocation || !formData.vehiclesType || !formData.description || !formData.policyId || !formData.categoryId) {
      alert('Please fill in all required fields: Start Location, End Location, Vehicle Type, Policy, Category, and Description')
      return
    }
            ...
            <CardContent className="px-6 py-4 space-y-2">
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    >
                      <SelectTrigger>
                        <SelectValue placeholder={loadingPolicies ? "Loading policies..." : "Select policy"} />
                      </SelectTrigger>
                      <SelectContent>
                        {policies.map((policy) => (
                  ...
                  <div className="space-y-2">
                    ...
                    <Select 
                      ...
                      disabled={(mode === 'view' && !editMode) || !selectedPolicy || loadingPolicies}
                    >
                      <SelectTrigger>
                        <SelectValue placeholder={!selectedPolicy ? "Select policy first" : "Select category"} />
                      </SelectTrigger>
                      <SelectContent>
              ...
              {(mode === 'create' || (mode === 'view' && editMode)) && (
                <div className="flex justify-end gap-2 pt-2">
                  ...
                  >
                    {saving ? 'Saving...' : (mode === 'view' ? 'Update Expense' : 'Submit Expense')}
                  </Button>
                </div>
              )}
            </CardContent>
