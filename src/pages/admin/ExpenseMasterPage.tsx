...
import { getTemplates, assignEntity, type Template } from '@/services/admin/templates'
...
const getEntityDisplayName = (assignedEntity: any, entityInfo: Entity | null): string => {
  return assignedEntity.display_name 
...
const getEmptyStateMessage = (entitiesLoading: boolean, assignedCount: number, totalCount: number): string => {
  if (entitiesLoading) {
...
const AssignedEntitiesList = ({ assignedEntities, entities }: AssignedEntitiesListProps) => {
              ...
              <div className="flex items-center gap-3">
                <div>
                  ...
                  {assignedEntity.description && (
                    <div className="text-xs text-gray-500">{assignedEntity.description}</div>
                  )}
                </div>
              </div>
...
const ExpenseMasterPage = () => {
  ...
  const assignedEntities = useMemo(() => {
    if (!expenseTemplate?.entities || !Array.isArray(expenseTemplate.entities)) {
      return []
    }
  ...
  const availableEntities = useMemo(() => {
    ...
    return entities.filter(entity => entity?.id && !assignedEntityIds.includes(entity.id))
  ...
  const handleSubmitCustom = async () => {
    ...
    if (!expenseTemplate?.id) {
      toast.error('Expense template not found. Please ensure templates are loaded.')
      return
    }
    ...
    try {
      const assignments = customFields.map(async (row) => {
        if (!row.entityId) return { success: false, entityId: '', error: 'No entity ID' }
      ...
      if (failed.length > 0) {
        toast.error(`Failed to assign ${failed.length} of ${results.length} entities`)
      }

      if (successful.length > 0) {
        toast.success(`Successfully assigned ${successful.length} ${successful.length === 1 ? 'entity' : 'entities'}`)
        setCustomFields([])
        await reloadData()
        setActiveTab('core')
      }
      ...
      toast.error(err?.response?.data?.message || 'Failed to assign custom fields')
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleSaveCoreConfig = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Failed to save core configuration')
    } finally {
      setLoading(false)
    }
  }
      ...
      <AdminLayout>
        ...
        {activeTab === 'core' ? (
          <Card className="p-6">
            ...
            {templates.length === 0 ? (
              <p className="text-sm text-gray-600 py-4">No templates found. Please refresh the page.</p>
            ) : (
              <div className="grid grid-cols-1 gap-4">
                ...
                {CORE_FIELDS.map((label) => (
                  <div key={label} className="grid grid-cols-2 gap-4 items-center p-3 rounded bg-white">
          ...
          <Card className="p-6">
            ...
            {templates.length === 0 ? (
              ...
              <div className="flex flex-col gap-4">
                    ...
                    <div key={idx} className="grid grid-cols-3 gap-4 items-center border-b pb-3 pt-3">
                      <div>
                        {entitiesLoading ? (
                          ...
                          <Select 
                            value={row.entityId} 
                            onValueChange={(v) => updateCustomField(idx, { entityId: v })} 
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex gap-2 flex-wrap">
                          {row.categories.length > 0 ? (
                            ...
                            <span className="text-xs text-gray-400 italic">Categories (read-only)</span>
                          )}
                        </div>
                        <Button variant="ghost" onClick={() => removeCustomField(idx)}>Remove</Button>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div className="mt-4">
                  <Button variant="link" onClick={addCustomField} className="text-sm text-blue-600">
                    Add Custom Field
                  </Button>
                </div>
